...
const MDVP_33_PARAMETER_SCHEMA=Object.freeze([
['F0','Average Fundamental Frequency','Hz','mean voiced-period F0','100–150 male; 180–240 female'],['Fhi','Highest Fundamental Frequency','Hz','maximum voiced-period F0','~212–293 Hz normative task range'],['Flo','Lowest Fundamental Frequency','Hz','minimum voiced-period F0','~148–257 Hz cohort dependent'],['STD','Standard Deviation of F0','Hz','SD of analyzed F0 periods','≤2.115 Hz'],['T0','Average Pitch Period','ms','1000/F0','inverse of F0'],['Jita','Absolute Jitter','µs','mean absolute consecutive period difference','≤83.200 µs'],['Jitt','Jitter Percent','%','relative period variability','≤1.040%'],['RAP','Relative Average Perturbation','%','3-period moving average perturbation','≤0.680%'],['PPQ','Pitch Period Perturbation Quotient','%','5-period moving average perturbation','≤0.840%'],['sPPQ','Smoothed Pitch Perturbation Quotient','%','55-period smoothing','≤1.020%'],['vF0','Coefficient of F0 Variation','%','100×SD(F0)/mean(F0)','≤1.100%'],['ShdB','Shimmer in dB','dB','absolute period amplitude variability','≤0.350 dB'],['Shim','Shimmer Percent','%','relative amplitude variability','≤3.810%'],['APQ','Amplitude Perturbation Quotient','%','11-period amplitude smoothing','≤3.070%'],['sAPQ','Smoothed Amplitude Perturbation Quotient','%','55-period amplitude smoothing','≤4.230%'],['vAm','Coefficient of Amplitude Variation','%','100×SD(amplitude)/mean(amplitude)','≤8.200%'],['NHR','Noise-to-Harmonic Ratio','ratio','inharmonic/harmonic energy','≤0.190'],['VTI','Voice Turbulence Index','ratio','high-frequency noise/harmonic energy','≤0.061'],['SPI','Soft Phonation Index','ratio','low-frequency harmonic/high-frequency energy','≤14.120'],['DVB','Degree of Voice Breaks','%','voice-break duration/sample duration','0.000%'],['DSH','Degree of Sub-Harmonic Components','%','sub-harmonic energy/period evidence','0.000%'],['DUV','Degree of Voiceless','%','relative unvoiced proportion','0.000%'],['NUV','Number of Unvoiced Segments','count','count unvoiced gaps','0'],['NVB','Number of Voice Breaks','count','count interruptions between voiced segments','0'],['NSH','Number of Sub-Harmonic Segments','count','count sub-harmonic segments','0'],['SEG','Total Number of Segments','count','computational frame segments','sample dependent'],['PER','Number of Pitch Periods','count','valid periodic cycles','sample dependent'],['PFR','Phonatory F0 Range','semitones','12×log2(Fhi/Flo)','<2.50 semitones'],['FTRI','Frequency Tremor Intensity Index','%','primary low-frequency F0 modulation magnitude','≤0.220%'],['ATRI','Amplitude Tremor Intensity Index','%','primary low-frequency amplitude modulation','≤2.860%'],['Fftr','F0 Tremor Frequency','Hz','dominant low-frequency F0 modulation','0 if below threshold'],['Fatr','Amplitude Tremor Frequency','Hz','dominant low-frequency amplitude modulation','0 if below threshold'],['Tsam','Length of Analyzed Data Sample','sec','duration of analyzed segment','1.00–3.00 s']
]);
...
function researchExtensionMetrics(track,periods,amplitudeRecords){
  const voiced=track.filter(x=>Number.isFinite(x.f0)&&x.f0>0), total=track.length;
  const unvoiced=total-voiced.length;
  let segments=0,inVoice=false;
  for(const x of track){if(Number.isFinite(x.f0)&&x.f0>0){if(!inVoice)segments++;inVoice=true}else inVoice=false}
  const ps=periods.filter(p=>Number.isFinite(p.periodSec)&&p.periodSec>0);
  const base=ps.length?median(ps.map(p=>p.periodSec)):null;
  const sub=base?ps.filter(p=>p.periodSec>base*1.65&&p.periodSec<base*2.35):[];
  const nsh=sub.length;
  const dsh=ps.length?100*nsh/ps.length:null;
  const amp=(amplitudeRecords||[]).map(p=>p.peakToPeak).filter(Number.isFinite);
  function modulation(values,times,lo=3,hi=12){
    if(values.length<8||times.length!==values.length)return{index:null,freq:null};
    const m=mean(values); if(!Number.isFinite(m)||m===0)return{index:null,freq:null};
    let best=-Infinity,bestHz=null;
    for(let hz=lo;hz<=hi;hz+=.5){
      let cs=0,sn=0;
      for(let i=0;i<values.length;i++){const a=2*Math.PI*hz*times[i],v=values[i]-m;cs+=v*Math.cos(a);sn+=v*Math.sin(a)}
      const amp2=2*Math.sqrt(cs*cs+sn*sn)/values.length;
      if(amp2>best){best=amp2;bestHz=hz}
    }
    return{index:100*best/m,freq:bestHz};
  }
  const fmod=modulation(voiced.map(x=>x.f0),voiced.map(x=>x.time));
  const amod=modulation(amp,amp.length===ps.length?ps.map(x=>x.time):[]);
  return {
    PFR:(voiced.length&&Math.min(...voiced.map(x=>x.f0))>0)?12*Math.log2(Math.max(...voiced.map(x=>x.f0))/Math.min(...voiced.map(x=>x.f0))):null,
    SEG:total,PER:ps.length,DUV:total?100*unvoiced/total:null,NUV:unvoiced,
    NVB:Math.max(0,segments-1),NSH:nsh,DSH:dsh,FTRI:fmod.index,ATRI:amod.index,Fftr:fmod.freq,Fatr:amod.freq
  };
}
...
function analyzeVoiceFast(samples,sr,task='vowel'){
  ...
  return {
    ...
    researchParameters:researchExtensionMetrics(track,periods,periods),
    measurementStatus:{
      core:'prototype-fast',
      mdvpComparable:['F0','Fhi','Flo','STD','T0','Jita','Jitt','RAP','PPQ','sPPQ','vF0','ShdB','Shim','APQ','sAPQ','vAm','NHR','VTI','SPI','DVB','DSH','DUV','NUV','NSH','NVB','FTRI','ATRI','Fftr','Fatr','SEG','PER','PFR'],
...
function analyzeVoice(samples,sr){
  ...
  const researchParameters=researchExtensionMetrics(track,periods,periods);
  const derivedT0Ms=Number.isFinite(f0Mean)&&f0Mean>0?1000/f0Mean:null;
  return {
    ...
    jitaUs:pf.jitaUs,jitterLocalPct:pf.jittPct,jittPct:pf.jittPct,rapPct:pf.rapPct,ppqPct:pf.ppqPct,
    sppqPct:pf.sppqPct,vf0Pct:pf.vf0Pct,t0Ms:derivedT0Ms,
    shimmerLocalPct:af.shimPct,shimPct:af.shimPct,shdB:af.shdB,apqPct:af.apqPct,sapqPct:af.sapqPct,vamPct:af.vamPct,
    nhr:noise.nhr,vti:noise.vti,spi:noise.spi,cppPrototypeDb:cpp,voicedPct,clippedPct:clipped,
    rmsDb:20*Math.log10(Math.max(r,1e-9)),peakDb:20*Math.log10(Math.max(p,1e-9)),
    ...
    pitchTrack:track,periods,periodLevel,researchParameters,
    measurementStatus:{
      core:'prototype',
