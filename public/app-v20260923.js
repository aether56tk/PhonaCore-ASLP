...
window.__PHONACORE_BUILD='2026-09-23-clinical-ux10';
...
function startNewAssessment(){
  ...
  state.page=state.patient?'Clinical':'Patients';
  state.notice=state.patient?'Continue the clinical assessment before recording.':'Create or select a real participant before starting an assessment.';
  render();
}
function dashboard(){const done=state.sessions.filter(s=>s.status==='completed'),q=done.map(s=>s.quality?.score).filter(Number.isFinite),hasPatient=!!state.patient;return `<div class="hero dashboardHero"><div><div class="eyebrow">PHONACORE-ASLP</div><h2>Voice assessment workspace</h2><p>Start by registering a real participant, complete the clinical information, then move to Voice Lab for recording and acoustic analysis.</p><div class="toolbar"><button class="btn primary" id="newA">＋ Start assessment</button>${hasPatient?`<button class="btn" id="continueDash">Continue with ${state.patient.name||state.patient.id}</button>`:''}</div></div></div>`+`<div class="dashboardStart"><div><span class="dashboardIcon">01</span><div><b>${hasPatient?'Participant selected':'No participant selected'}</b><small>${hasPatient?'Continue to the clinical assessment.':'Register a real participant before recording.'}</small></div></div><div class="dashboardHint">${hasPatient?'Ready for the next step.':'Nothing has been recorded yet.'}</div></div>`+`<div class="grid"><div class="stat"><span>Completed sessions</span><b>${done.length}</b><small>local records</small></div><div class="stat"><span>Mean quality</span><b>${q.length?Math.round(mean(q)):'—'}</b><small>${q.length?'out of 100':'no completed sessions'}</small></div><div class="stat"><span>Participants</span><b>${state.patients.length}</b><small>real registry</small></div><div class="stat"><span>Research samples</span><b>${state.sessions.length}</b><small>saved session records</small></div></div>`}
function patients(){
...
function occupationHygieneKey(occupation=''){const o=String(occupation).toLowerCase();if(/teacher|lecturer|professor|school|faculty/.test(o))return'Teacher';if(/lawyer|advocate|attorney|court/.test(o))return'Lawyer';if(/call.?center|call centre|telecall|customer service|bpo/.test(o))return'CallCenter';if(/singer|vocalist|performer|musician/.test(o))return'Singer';if(/speaker|trainer|presenter|anchor|host/.test(o))return'PublicSpeaker';return'General'}function analysisView(a){const p=state.patient||{},h=store.get('history_'+p.id,{}),occupation=h.occupation||'Not documented',profile=hygieneProfiles[occupationHygieneKey(h.occupation)]||hygieneProfiles.General;const pts=(a.pitchTrack||[]).filter(x=>x.f0).map(x=>x.f0);const rows=[['F0 mean',a.f0Mean,'Hz'],['F0 minimum',a.f0Min,'Hz'],['F0 maximum',a.f0Max,'Hz'],['F0 SD',a.f0Sd,'Hz'],['T0',a.t0Ms,'ms'],['Jita',a.jitaUs,'µs'],['Jitt',a.jittPct,'%'],['RAP',a.rapPct,'%'],['PPQ',a.ppqPct,'%'],['sPPQ',a.sppqPct,'%'],['vF0',a.vf0Pct,'%'],['ShdB',a.shdB,'dB'],['Shim',a.shimPct,'%'],['APQ',a.apqPct,'%'],['sAPQ',a.sapqPct,'%'],['vAm',a.vamPct,'%'],['NHR',a.nhr,'ratio'],['VTI',a.vti,'ratio'],['SPI',a.spi,'ratio'],['CPP*',a.cppPrototypeDb,'dB-like']];return card('Final voice analysis output',`<div class="patient"><b>Participant</b><small>${p.name||'—'} • ${p.id||'—'} • Occupation: ${occupation}</small></div><div class="metrics">${rows.map(r=>'<div class="metric"><span>'+r[0]+'</span><b>'+f(r[1])+'</b><small>'+r[2]+'</small></div>').join('')}</div><p class="small">Duration ${f(a.durationSec)} s • Voiced ${f(a.voicedPct)}% • Clipping ${f(a.clippedPct)}%</p><div class="chart">${line(pts)}</div><div class="notice">${a.quality.issues.length?a.quality.issues.join(' • '):'Audio processed successfully. No basic capture-quality flags detected.'}<br><small>*CPP is a research implementation and requires independent validation before clinical use.</small></div>`)+card('Vocal hygiene & occupation-specific tips',`<div class="patient"><b>${profile.title}</b><small>Occupation entered: ${occupation} • Focus: ${profile.focus}</small></div><ul class="clean">${profile.items.map(x=>'<li>☐ '+x+'</li>').join('')}</ul><div class="notice">These are educational voice-care recommendations based on the recorded occupation. They do not diagnose a voice disorder or replace individualized ENT/SLP advice.</div>`)+card('Session actions',`<div class="toolbar">${btn('Save session','saveSession')}${btn('Export JSON','exportJSON')}</div>`)}
...
function printableReport(s,p,h){const w=window.open('','_blank');if(!w)return;w.document.write('<!doctype html><html><head><title>PhonaCore-ASLP Report '+s.id+'</title><style>body{font-family:Arial,sans-serif;max-width:900px;margin:30px auto;line-height:1.45;color:#111}h1,h2{margin-bottom:6px}.meta{padding:12px;border:1px solid #ccc;margin:12px 0}table{width:100%;border-collapse:collapse}th,td{border:1px solid #ccc;padding:7px;text-align:left}.note{padding:12px;background:#f5f5f5}</style></head><body><h1>PhonaCore-ASLP Voice Assessment Report</h1><p>Research/educational assessment report — clinician interpretation required.</p><div class="meta"><b>Patient:</b> '+(p?.name||'—')+' ('+(p?.id||'—')+')<br><b>Assessment:</b> '+s.id+'<br><b>Date:</b> '+new Date(s.createdAt).toLocaleString()+'<br><b>Task:</b> '+s.task+'<br><b>Sample rate:</b> '+(s.recordingMeta?.sampleRate||'—')+' Hz</div><h2>Clinical summary</h2><p><b>Concern:</b> '+(h.concern||'Not documented')+'</p><p><b>Occupation/voice load:</b> '+(h.occupation||'Not documented')+' / '+(h.load||'Not documented')+'</p><p><b>Symptoms:</b> '+((h.symptoms||[]).join(', ')||'None documented')+'</p><h2>Acoustic measurements</h2>'+parameterTable(s.m)+'<h2>Signal quality</h2><p>Score: '+s.quality.score+'/100. '+((s.quality.issues||[]).join(' • ')||'No basic quality flags detected.')+'</p><div class="note">Measurements are descriptive. They do not independently establish a diagnosis, normality, or clinical equivalence with MDVP.</div><h2>Clinician observations</h2><p>'+(h.perceptual||'Not documented')+'</p><h2>Follow-up</h2><p>'+(h.follow||'Not documented')+'</p><script>window.onload=()=>window.print()</script></body></html>');w.document.close()}
...
function pairedValidation(phona,mdvp){const map=new Map(phona.map(x=>[String(x.sample_id),x]));const rows=mdvp.map(m=>({sample_id:String(m.sample_id),phona:map.get(String(m.sample_id))||null,mdvp:m}));const duplicateIds=mdvp.map(x=>String(x.sample_id)).filter((x,i,a)=>a.indexOf(x)!==i);const params=[['F0','f0Mean'],['Fhi','f0Max'],['Flo','f0Min'],['STD','f0Sd'],['Jita','jitaUs'],['Jitt','jittPct'],['RAP','rapPct'],['PPQ','ppqPct'],['sPPQ','sppqPct'],['vF0','vf0Pct'],['ShdB','shdB'],['Shim','shimPct'],['APQ','apqPct'],['sAPQ','sapqPct'],['vAm','vamPct'],['NHR','nhr']];return{rows,duplicateIds,params}}
...
function wire(){
  ...
  on('runBatch','click',async()=>{
    ...
    for(const file of files){
      try{
        ...
        const row={sample_id:file.name.replace(/\.[^.]+$/,''),fileName:file.name,sampleRate:d.sampleRate,channels:d.channels,durationSec:d.durationSec,...a,measurementGate,sha256};
    ...
    $('#batchManifest').textContent=JSON.stringify({generatedAt:new Date().toISOString(),count:rows.length,analysis:'PhonaCore-ASLP',localProcessing:true,protocol:'STANDARD_PROTOCOL',samples:rows.map(x=>({sample_id:x.sample_id,fileName:x.fileName,sha256:x.sha256,sampleRate:x.sampleRate,channels:x.channels,durationSec:x.durationSec,protocolStatus:x.protocolStatus,protocolIssues:x.protocolIssues||[]}))},null,2);
  });
  ...
  on('makeManifest','click',()=>{
    const m={protocolVersion:'1.0',studyId:$('#studyId').value,sampleId:$('#sampleId').value,participantCode:$('#participantCode').value,device:$('#studyDevice').value,task:'sustained /a/',targetDurationSec:6,acceptableDurationSec:[5,10],channels:1,microphoneDistanceCm:15,angleDegrees:45,preprocessing:{echoCancellation:false,noiseSuppression:false,autoGainControl:false},createdAt:new Date().toISOString()};
