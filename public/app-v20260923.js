...
const nav=['Dashboard','Patients','Clinical','Voice Lab','Research Lab','File Manager','Settings'];
...
function research(){return head('Research Lab','Validation, agreement and reproducibility analysis.')+card('MDVP Validation Lab',`<div class="drop"><p><b>Reference comparison:</b> import PhonaCore JSON and MDVP CSV with matching <code>sample_id</code>.</p><input type="file" id="phonaFile" accept=".json,application/json"><input type="file" id="mdvpFile" accept=".csv,text/csv"><div class="toolbar"><button class="btn primary" id="runMdvp">Run validation</button><button class="btn" id="clearMdvp">Clear</button></div><p class="small">Parameters: F0, Fhi, Flo, STD, Jita, Jitt, RAP, PPQ, sPPQ, vF0, ShdB, Shim, APQ, sAPQ, vAm, NHR.</p></div><div id="mdvpResult"></div>`)+card('Synthetic benchmark',`<p>Run deterministic generated-voice tests against mathematical ground truth.</p><button class="btn primary" id="runSynthetic">Run benchmark</button><pre id="syntheticOut"></pre>`)+card('Reproducibility study',`<p>Paste paired numeric measurements to calculate agreement statistics.</p><textarea id="reproData" rows="8" placeholder="reference,phonaCore\n120,121\n119,120"></textarea><div class="toolbar"><button class="btn primary" id="runRepro">Analyze</button><button class="btn" id="exportRepro">Export JSON</button></div><div id="reproOut"></div>`)+card('Experiment',`<button class="btn primary" id="newExp">Create experiment</button><pre id="researchOut">${JSON.stringify(state.experiments,null,2)}</pre>`)}
