...
window.__PHONACORE_BUILD='2026-09-24-final4-role-worker-hygiene';
...
let state={page:initialRole==='patient'?'Demographics':initialRole==='clinician'?'Dashboard':'Role Selection',role:initialRole,researchAudit:store.get('researchAudit',[]),mode:store.get('mode','clinical'),theme:store.get('theme','night'),patient:null,patients:store.get('patients',[]),sessions:store.get('sessions',[]),recording:false,stream:null,recorder:null,chunks:[],pcmChunks:[],pcmProcessor:null,timer:null,seconds:0,analysis:null,tele:null,datasets:store.get('datasets',[]),experiments:store.get('experiments',[]),audioFiles:[],fileDirectory:null,pendingRecordingId:null};
...
function wire(){
  ...
  on('chooseClinician','click',()=>chooseRole('clinician'));
  on('choosePatient','click',()=>chooseRole('patient'));
