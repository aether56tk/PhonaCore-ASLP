...
window.__PHONACORE_BUILD='2026-09-24-uiux-role-redesign-01';
...
const initialRole=store.get('role',null);
let state={page:initialRole==='patient'?'Demographics':initialRole==='clinician'?'Dashboard':'Role Selection',role:initialRole,pendingRole:initialRole,researchAudit:store.get('researchAudit',[]),mode:store.get('mode','clinical'),theme:store.get('theme','night'),patient:null,patients:store.get('patients',[]),sessions:store.get('sessions',[]),recording:false,stream:null,recorder:null,chunks:[],pcmChunks:[],pcmProcessor:null,timer:null,seconds:0,analysis:null,tele:null,datasets:store.get('datasets',[]),experiments:store.get('experiments',[]),audioFiles:[],fileDirectory:null,pendingRecordingId:null};
...
function navIcon(n){return{Dashboard:'⌂',Patients:'♙',Clinical:'✚','Voice Lab':'〽','Research Lab':'⚗',Recommendations:'♥',Report:'▤','MDVP Validation Dashboard':'▥','File Manager':'▣'}[n]||'•'}function render(){const nav=roleNav();document.body.dataset.theme=state.theme;document.body.innerHTML=`<div class="layout"><aside class="side"><div class="brand"><span class="brandMark">〽</span><span>Phona<span>Core</span><small>ASLP • VOICE RESEARCH</small></span></div><nav class="navList">${nav.map(n=>`<button class="nav ${state.page===n?'active':''}" data-page="${n}"><i>${navIcon(n)}</i><span>${n}</span></button>`).join('')}</nav>${state.role?'<button class="btn roleSwitch" id="switchRole">⇄ Switch role</button>':''}</aside><main class="main"><div class="top"><span>${state.page==='Role Selection'?'PhonaCore-ASLP':'PhonaCore-ASLP / '+state.page}</span><div class="topActions">${state.page==='Role Selection'?'<span class="buildPill">LOCAL • PRIVATE</span>':'<button class="btn primary" id="globalNewAssessment">＋ New assessment</button>'}<button class="btn" id="themeToggle">${state.theme==='night'?'☀ Daylight':'☾ Night'}</button></div></div><section class="content">${page()}</section></main>${state.notice?`<div class="toast">${state.notice}</div>`:''}</div>`;document.querySelectorAll('[data-page]').forEach(b=>b.onclick=()=>{state.page=b.dataset.page;render()});wire()}
...
function reportPage(){
 ...
 const screenRules=[['Jitt','jittPct',1.040,'%'],['RAP','rapPct',0.680,'%'],['PPQ','ppqPct',0.840,'%'],['sPPQ','sppqPct',1.020,'%'],['vF0','vf0Pct',1.100,'%'],['ShdB','shdB',0.350,'dB'],['Shim','shimPct',3.810,'%'],['APQ','apqPct',3.070,'%'],['sAPQ','sapqPct',4.230,'%'],['vAm','vamPct',8.200,'%'],['NHR','nhr',0.190,'ratio'],['VTI','vti',0.061,'ratio'],['SPI','spi',14.120,'ratio']];
 const applicableScreen=screenRules.filter(x=>Number.isFinite(Number(m[x[1]]))),failedScreen=applicableScreen.filter(x=>Number(m[x[1]])>x[2]);
 const gateOverall=m.measurementStatus?.gate?.overall||'limited';
 const screeningStatus=gateOverall!=='valid'||!applicableScreen.length?'REVIEW':failedScreen.length?'FAIL':'PASS';
 const screeningText=screeningStatus==='PASS'?'No applicable configured reference was exceeded.':screeningStatus==='FAIL'?'One or more configured reference values were exceeded.':'Signal/measurement quality or available values require review before assigning PASS or FAIL.';
 ...
 card('1 · Patient & assessment','<div class="metaGrid"><div><b>Participant</b><span>'+ (p?.name||'—')+'</span></div><div><b>ID</b><span>'+ (p?.id||'—')+'</span></div><div><b>Occupation</b><span>'+ (h.occupation||'Not documented')+'</span></div><div><b>Assessment</b><span>'+s.id+'</span></div><div><b>Date</b><span>'+new Date(s.createdAt||Date.now()).toLocaleString()+'</span></div><div><b>Task</b><span>'+ (s.task||m.task||'—')+'</span></div></div>')+card('2 · Screening reference outcome','<div class="clinicalOutcome '+screeningStatus.toLowerCase()+'"><div class="outcomeBadge">'+screeningStatus+'</div><div><b>'+screeningText+'</b><p class="small">This is a configured screening/reference result, not a diagnosis. Review recording quality, history and clinical findings before interpretation.</p></div></div>')+
 card('3 · Signal quality & acquisition',signalQualityPanel(m)+ '<div class="notice">Review signal-quality flags before clinical or research interpretation.</div>')+
 card('4 · Acoustic measurements',parameterTable(m))+
 card('5 · Signal visualization','<h3>F0 contour</h3><div class="chart">'+line((m.pitchTrack||[]).filter(x=>x.f0).map(x=>x.f0))+'</div><h3>Waveform preview</h3><div class="chart">'+(m.waveformPreview?.length?'<svg viewBox="0 0 800 220" preserveAspectRatio="none">'+(()=>{const v=m.waveformPreview,max=Math.max(...v.map(Math.abs))||1;return '<polyline fill="none" stroke="currentColor" stroke-width="1.5" points="'+v.map((x,i)=>(i/(v.length-1||1)*800)+','+(110-x/max*95)).join(' ')+'"/>'})()+'</svg>':'<div class="empty">Waveform preview was not retained for this assessment.</div>')+'</div><p class="small">A true spectrogram is shown only when raw signal frames are retained; PhonaCore does not substitute a synthetic image for missing signal data.</p>')+
 card('6 · Voice profile',radialProfile(m))+
 card('7 · Research fingerprint','<pre class="fingerprint">'+JSON.stringify(fp,null,2).replace(/</g,'&lt;')+'</pre>')+
...
function roleSelection(){
 ...
 return '<div class="onboard"><div class="onboardMain"><div class="onboardTop"><div class="stepLine"><span class="stepActive"></span><span></span><span></span></div><span class="stepText">01 <i>/ 03</i></span></div><div class="onboardTitle"><span class="eyebrow">PHONACORE • VOICE RESEARCH</span><h1>Tell us how you’ll use<br>PhonaCore</h1><p>Choose the workspace that matches your role. You can switch roles later.</p></div><div class="roleChoices"><button class="roleChoice '+(selected==='clinician'?'selected':'')+'" id="chooseClinician"><span class="radioDot"></span><span class="roleChoiceIcon">🩺</span><span class="roleChoiceText"><b>Clinician</b><small>Patient assessment, Voice Lab, reports, recommendations and research tools.</small></span><span class="choiceArrow">›</span></button><button class="roleChoice '+(selected==='patient'?'selected':'')+'" id="choosePatient"><span class="radioDot"></span><span class="roleChoiceIcon">🎙️</span><span class="roleChoiceText"><b>Patient / Other User</b><small>Personal voice analysis, simplified reports, recommendations and voice-hygiene tools.</small></span><span class="choiceArrow">›</span></button></div><div class="onboardActions"><button class="btn onboardBack" id="roleBack" '+(selected?'':'disabled')+'>Back</button><button class="btn primary onboardNext" id="roleNext" '+(selected?'':'disabled')+'>Continue <span>→</span></button></div><div class="onboardPrivacy"><span>●</span><div><b>Privacy-first</b><small>Voice analysis is processed locally in this browser. No account is required for the basic workflow.</small></div></div></div><div class="onboardVisual"><div class="visualGlow"></div><div class="visualLabel">VOICE ANALYSIS WORKSPACE</div><div class="visualTitle">Objective voice<br><em>measurement</em></div><div class="visualPanel"><div class="visualPanelTop"><span>PhonaCore</span><span class="visualPill">LIVE</span></div><div class="visualWave">'+Array.from({length:12},(_,i)=>'<i style="--h:'+(28+((i*17)%54))+'%"></i>').join('')+'</div><div class="visualMetrics"><div><small>F₀</small><b>— Hz</b></div><div><small>Jitt</small><b>— %</b></div><div><small>Shim</small><b>— %</b></div></div><div class="visualFooter"><span>Research ready</span><span>Acoustic • MDVP</span></div></div><div class="visualOrbs"><span></span><span></span><span></span></div></div><div class="onboardNotice"><b>Important</b><span>Patient-facing results are screening references, not diagnoses. Persistent voice concerns should be assessed by an appropriate clinician.</span></div></div>';
}
...
function researchConsole(){
 ...
 card('Research tools','<div class="toolGrid"><button class="toolButton" data-page="Pre-Collection Research Gate"><span>01</span><b>Pre-Collection Gate</b><small>Study readiness and collection lock</small></button><button class="toolButton" data-page="Research Wizard"><span>02</span><b>Research Wizard</b><small>Guided research workflow</small></button><button class="toolButton" data-page="MDVP Validation Dashboard"><span>03</span><b>MDVP Validation</b><small>Reference comparison workflow</small></button><button class="toolButton" data-page="Research Validation Suite"><span>04</span><b>Validation Suite</b><small>Validation and concordance tools</small></button><button class="toolButton" data-page="DSP Accuracy Lab"><span>05</span><b>Accuracy Lab</b><small>Algorithm ground-truth QA</small></button><button class="toolButton" data-page="Research Integrity Gate"><span>06</span><b>Integrity Gate</b><small>Completeness and provenance</small></button><button class="toolButton" data-page="Data Integrity & Readiness"><span>07</span><b>Data Integrity</b><small>Dataset readiness checks</small></button><button class="toolButton" data-page="Full System Verification"><span>08</span><b>System Verification</b><small>End-to-end system checks</small></button></div>')+
...
function patients(){
  ...
  return head('Patients','Register and select a real participant before starting a clinical assessment.')+card('Participant details',form)+card('Participant registry',list)+card('Selected participant',selectedView);
}
...
function clinicianVoiceLab(){
 if(!state.patient)return head('Voice Lab','Select a real participant before standardized acquisition.',btn('Go to Patients','goPatients'))+card('Participant required','<div class="empty">A clinical recording must be linked to a real participant. No sample patient is created automatically.</div>');
 const a=state.analysis,task=state.voiceTask||'vowel';
 const opts=[['vowel','Sustained vowel'],['reading','Reading'],['counting','Counting'],['conversation','Conversation'],['mpt','Maximum phonation time'],['pitchrange','Pitch range'],['intensity','Loudness / intensity']];
 const processing=!state.recording&&!a&&/Analyzing|analysis|processing/i.test(state.notice||'');
 const status=state.recording?'RECORDING':processing?'PROCESSING':a?'ANALYZED':'READY';
 return head('Voice Lab','Clinician acquisition workspace — standardized capture, quality gate and objective analysis.', '<span class="tag">'+status+'</span>')+
 '<div class="voiceLabHero"><div><span class="eyebrow">CLINICIAN WORKSPACE • VOICE ACQUISITION</span><h2>'+((state.patient.name||state.patient.id))+'</h2><p>'+state.patient.id+' • '+(state.patient.age||'—')+' years • '+(state.patient.sex||'—')+' • '+(task==='vowel'?'Sustained vowel':'Task: '+task)+'</p></div><div class="voiceLabStatus '+(state.recording?'recording':processing?'processing':'ready')+'"><small>WORKFLOW STATUS</small><b>'+status+'</b><small>'+(state.recording?'Microphone active':processing?'Audio analysis in progress':a?'Analysis available for review':'Ready to acquire a sample')+'</small></div></div>'+
 '<div class="voiceLabToolbar"><label class="voiceTaskSelect"><span>Assessment task</span><select id="task">'+opts.map(x=>'<option value="'+x[0]+'" '+(task===x[0]?'selected':'')+'>'+x[1]+'</option>').join('')+'</select></label>'+(state.recording?btn('■ Stop recording','stop'):btn('● Start recording','record'))+btn('Reset','reset')+(a?'<button class="btn" onclick="state.page=\'Report\';render()">Open clinical report →</button>':'')+'</div>'+
 taskGuide(task)+
 card('Live acquisition','<div class="recordingConsole"><div><div class="recordingTimer">'+String(Math.floor(state.seconds/60)).padStart(2,'0')+':'+String(state.seconds%60).padStart(2,'0')+'</div><small>ELAPSED</small></div><div><div class="recordingState"><span class="recordingStateDot '+(state.recording?'live':processing?'processing':'')+'"></span><span>'+(state.recording?'Recording in progress':processing?'Processing recording':a?'Recording analyzed':'Ready to record')+'</span></div><div class="recordingMeta">'+(state.notice||'Use the selected protocol and record without deliberately altering the voice.')+'</div><div class="level" style="margin-top:13px"><i id="level"></i></div><div class="captureWave">'+Array.from({length:42},(_,i)=>'<i style="--h:'+(18+((i*23)%66))+'%"></i>').join('')+'</div></div></div>')+
 (a?clinicianAnalysisWorkspace(a):card('Standardized acquisition protocol','<div class="protocolSteps"><div><b>01</b><span>Quiet environment</span><small>Minimize background noise and interruptions.</small></div><div><b>02</b><span>Stable setup</span><small>Keep microphone position and device consistent.</small></div><div><b>03</b><span>Comfortable voice</span><small>Use habitual pitch and loudness; do not force the voice.</small></div><div><b>04</b><span>Quality review</span><small>Check the measurement gate before interpretation.</small></div></div><div class="notice" style="margin-top:13px">This clinician workspace contains detailed acoustic and research outputs. Patient users receive a separate simplified voice view.</div>'));
}
function clinicianAnalysisWorkspace(a){
 const rows=[['F₀ mean',a.f0Mean,'Hz'],['F₀ min',a.f0Min,'Hz'],['F₀ max',a.f0Max,'Hz'],['F₀ SD',a.f0Sd,'Hz'],['T0',a.t0Ms,'ms'],['Jita',a.jitaUs,'µs'],['Jitt',a.jittPct,'%'],['RAP',a.rapPct,'%'],['PPQ',a.ppqPct,'%'],['sPPQ',a.sppqPct,'%'],['vF0',a.vf0Pct,'%'],['ShdB',a.shdB,'dB'],['Shim',a.shimPct,'%'],['APQ',a.apqPct,'%'],['sAPQ',a.sapqPct,'%'],['vAm',a.vamPct,'%'],['NHR',a.nhr,'ratio'],['VTI',a.vti,'ratio'],['SPI',a.spi,'ratio'],['CPP*',a.cppPrototypeDb,'dB-like']];
 const rp=a.researchParameters||{};
 const researchRows=[['PFR',rp.PFR,'semitones'],['DVB',rp.DVB,'%'],['DSH',rp.DSH,'%'],['DUV',rp.DUV,'%'],['NUV',rp.NUV,'count'],['NSH',rp.NSH,'count'],['NVB',rp.NVB,'count'],['FTRI',rp.FTRI,'%'],['ATRI',rp.ATRI,'%'],['Fftr',rp.Fftr,'Hz'],['Fatr',rp.Fatr,'Hz'],['SEG',rp.SEG,'count'],['PER',rp.PER,'count']];
 return '<div class="voiceLabGrid"><div>'+
 card('Measurement overview','<div class="analysisHeader"><div><h2>Objective acoustic measurements</h2><small>Clinician review • descriptive outputs • configured references</small></div><span class="tag">CLINICIAN ONLY</span></div><div class="analysisMetricGrid">'+[['F₀',a.f0Mean,'Hz'],['Jitter',a.jittPct,'%'],['Shimmer',a.shimPct,'%'],['NHR',a.nhr,'ratio']].map(x=>'<div class="analysisMetric"><span>'+x[0]+'</span><b>'+f(x[1])+'</b><small>'+x[2]+'</small></div>').join('')+'</div>'+
 signalQualityPanel(a)+
 '<div class="tablewrap" style="margin-top:14px"><table><thead><tr><th>Measurement</th><th>Value</th><th>Unit</th></tr></thead><tbody>'+rows.map(r=>'<tr><td><b>'+r[0]+'</b></td><td>'+f(r[1])+'</td><td>'+r[2]+'</td></tr>').join('')+'</tbody></table></div>'+
 '<div class="researchOnlyPanel" style="margin-top:14px"><span class="tag">RESEARCH OUTPUTS</span><span class="tag">NOT PATIENT-FACING</span><p>Extended MDVP-oriented parameters are retained for clinician/research review and validation workflows. They should not be presented as a patient diagnosis.</p></div>'+
 '<div class="tablewrap" style="margin-top:12px"><table><thead><tr><th>Research / MDVP extension</th><th>Value</th><th>Unit</th></tr></thead><tbody>'+researchRows.map(r=>'<tr><td>'+r[0]+'</td><td>'+f(r[1])+'</td><td>'+r[2]+'</td></tr>').join('')+'</tbody></table></div></div>'+
 card('Signal visualization','<div class="chart">'+line((a.pitchTrack||[]).filter(x=>x.f0).map(x=>x.f0))+'</div><p class="small">F₀ contour from detected pitch frames. Review alongside signal-quality status.</p>')+
 '</div><div>'+
 card('Participant & workflow','<div class="patient"><b>'+((state.patient.name||state.patient.id))+'</b><small>'+state.patient.id+' • '+(state.patient.age||'—')+' years • '+(state.patient.sex||'—')+'</small></div><div class="notice">Recording source, task and quality metadata remain linked to this participant record.</div>')+
 card('Research profile',radialProfile(a))+
 card('Next clinical actions','<div class="toolbar">'+btn('Save session','saveSession')+btn('Export JSON','exportJSON')+btn('Generate PDF','printFinalReport')+'</div><div class="notice">Use the clinical report for documented interpretation. Research validation tools remain in the clinician Research Console.</div>')+
 '</div></div>';
}function patientVoiceLab(){
 if(!state.patient)return head('Voice Check','Complete your profile before recording.');
 const a=state.analysis,task=state.voiceTask||'vowel';
 const opts=[['vowel','Sustained vowel'],['reading','Reading'],['conversation','Conversation']];
 return head('Voice Check','Simple voice recording for your personal review.', '<span class="tag">'+(state.recording?'RECORDING':'READY')+'</span>')+
 '<div class="patientVoiceHero"><div><span class="eyebrow">PERSONAL VOICE CHECK</span><h2>Record your voice</h2><p>Choose a simple task, record comfortably, and review your personal screening summary.</p></div><div class="patientVoiceLock">🔒<small>Personal view</small></div></div>'+
 '<div class="toolbar patientVoiceToolbar"><label class="taskSelect"><span>Recording task</span><select id="task">'+opts.map(x=>'<option value="'+x[0]+'" '+(task===x[0]?'selected':'')+'>'+x[1]+'</option>').join('')+'</select></label>'+
 (state.recording?btn('■ Stop recording','stop'):btn('● Start recording','record'))+btn('Reset','reset')+'</div>'+
 card('Recording','<div class="patientCapture"><div class="timer">'+String(Math.floor(state.seconds/60)).padStart(2,'0')+':'+String(state.seconds%60).padStart(2,'0')+'</div><div><b>'+(state.recording?'Recording in progress':'Ready when you are')+'</b><p class="small">'+(state.notice||'Use a quiet place and speak naturally.')+'</p><div class="level"><i id="level"></i></div></div></div>')+
 (a?patientAnalysisSummary(a):card('Before you record','<div class="patientSteps"><div><b>01</b><span>Find a quiet place</span></div><div><b>02</b><span>Keep a comfortable voice</span></div><div><b>03</b><span>Record without forcing your voice</span></div></div><div class="notice">Your personal results are simplified screening information. They are not a diagnosis.</div>'));
}
function patientAnalysisSummary(a){
 const gate=a.measurementStatus?.gate||{}, status=gate.overall==='valid'?'READY FOR REVIEW':'REVIEW RECORDING';
 const metrics=[['F₀',a.f0Mean,'Hz'],['Jitter',a.jittPct,'%'],['Shimmer',a.shimPct,'%'],['NHR',a.nhr,'ratio']];
 return card('Your voice summary','<div class="patientSummaryTop"><div class="patientSummaryBadge">'+(gate.overall==='valid'?'✓':'!')+'</div><div><h3>'+status+'</h3><p class="small">These measurements are shown for personal screening only.</p></div></div><div class="metrics patientMetrics">'+metrics.map(x=>'<div class="metric"><span>'+x[0]+'</span><b>'+f(x[1])+'</b><small>'+x[2]+'</small></div>').join('')+'</div><div class="toolbar"><button class="btn primary" onclick="state.page=\'Report\';render()">View my report →</button><button class="btn" onclick="state.page=\'Recommendations\';render()">Voice-care guidance</button></div><div class="notice">A screening result does not establish a voice disorder. If you have persistent voice concerns, discuss them with an appropriate clinician.</div>');
}
...
function signalQualityPanel(a){
 const m=a.measurementStatus||{},issues=m.issues||m.gate?.issues||[],g=m.gate||{},overall=g.overall||m.overall||'—';
 const label=overall==='valid'?'PASS':overall==='limited'?'LIMITED':String(overall).toUpperCase();
 const tone=overall==='valid'?'ok':overall==='limited'?'warn':'';
 return '<div class="qualityPanel"><div class="qualityGrid"><div class="metric qualityStatus '+tone+'"><span>Measurement gate</span><b>'+label+'</b><small>'+issues.length+' issue(s)</small></div><div class="metric"><span>Voiced material</span><b>'+f(a.voicedPct)+'%</b><small>current recording</small></div><div class="metric"><span>Clipping</span><b>'+f(a.clippedPct)+'%</b><small>signal saturation</small></div><div class="metric"><span>Pitch periods</span><b>'+f(a.periodCount)+'</b><small>detected cycles</small></div></div>'+(issues.length?'<div class="notice"><b>Review before interpretation:</b><ul class="clean">'+issues.map(x=>'<li>'+x+'</li>').join('')+'</ul></div>':'<div class="notice successNotice">No current measurement-gate issues were reported.</div>')+'</div>';
}
...
async function finishPCM(){
  try{
    ...
    const worker=new Worker('./analysis-worker.js?build=20260924-worker-fix1');
...
function chooseRole(role){
 state.role=role;state.pendingRole=role;store.set('role',role);
 if(role==='patient'){state.patient=null;state.page='Demographics';}
 else {state.patient=null;state.page='Dashboard';}
 state.notice=role==='patient'?'Patient / other-user interface selected.':'Clinician interface selected.';
 render();
}
function selectRole(role){state.pendingRole=role;render()}
function continueRole(){if(state.pendingRole)chooseRole(state.pendingRole)}

function wire(){
  const on=(id,event,handler)=>{const el=$('#'+id);if(el)el.addEventListener(event,handler);};
  on('chooseClinician','click',()=>selectRole('clinician'));
  on('choosePatient','click',()=>selectRole('patient'));
  on('roleNext','click',continueRole);
  on('roleBack','click',()=>{state.pendingRole=null;render()});
  on('switchRole','click',()=>{state.role=null;state.patient=null;state.page='Role Selection';try{localStorage.removeItem('sv_role')}catch(_){}try{sessionStorage.removeItem('sv_role')}catch(_){}render()});
  ...
  on('saveSelfProfile','click',()=>{const name=$('#pName')?.value.trim(),age=Number($('#pAge')?.value),sex=$('#pSex')?.value;if(!name||!Number.isFinite(age)||age<0||age>120||!sex){state.notice='Enter your name, valid age and sex.';render();return}state.patient={id:'SELF-'+Date.now().toString(36).toUpperCase(),name,age,sex};state.page='Voice Lab';state.notice='Profile saved. Voice Lab is ready.';render()});
  on('scoreHygiene','click',()=>{const checks=[...document.querySelectorAll('.hygieneCheck:checked')].length,total=document.querySelectorAll('.hygieneCheck').length,score=total?Math.round(checks/total*100):0;const el=$('#hygieneScore');if(el)el.innerHTML='<div class="result '+(score>=70?'pass':'review')+'"><span>'+score+'%</span><small>'+checks+' of '+total+' voice-care habits selected.</small></div>';state.notice='Voice-hygiene self-check scored '+score+'%.';});
